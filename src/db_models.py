...
class Theme(db.Model):
    ...
    anime = db.relationship('Anime', backref=db.backref('themes', lazy=True))

    artist_id = db.Column(db.Integer, db.ForeignKey('artists.id'), nullable=True)
    artist = db.relationship('Artist', backref=db.backref('themes', lazy=True))
    ...
    def json(lst):
        ...
        for it in lst:
            mirror_list = []
            for mirror_index, mirror in enumerate(it.mirrors):
                mirror['audio'] = f"{base_url}/{it.slug.split('-')[0]}/{it.slug.split('-')[1]}/{mirror_index}/audio"
                mirror['quality'] = mirror['quality']
