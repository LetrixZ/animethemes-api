...
from api.models import Theme, Anime
...
def get_entry(anime):
    themes = Theme.objects.filter(mal_id=anime.mal_id).all()
    theme_list = []
    for theme in themes:
        theme_list.append(theme.v1_json())
    new_list = sorted(theme_list, key=itemgetter('theme_id'), reverse=False)
    return {'mal_id': anime.mal_id, 'title': anime.title, 'cover': anime.cover,
            'season': anime.season,
